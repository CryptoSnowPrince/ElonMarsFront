...
import { ADMIN_WALLET_ADDRESS, chainId, URANIUM_MINE } from "../../hook/constants";
...
const UraniumMineModal = ({open, setOpen}:Props) => {
  ...
  const onBuyMining = async () => {
    ...
    let transaction = await deposit(address, ADMIN_WALLET_ADDRESS[chainId], URANIUM_MINE.COST.toString());
    dispatch(buyMining(address, URANIUM_MINE.COST, transaction.transactionHash, "uranium", (res:any)=>{
      if(res.success) {
  ...
  const onClaimMining = async () => {
    dispatch(claimMining(address, "uranium", (res:any)=>{handleClose()}));
  }

  const onRequestMining = async () => {
    dispatch(requestMining(address, "uranium", (res:any)=>{handleClose()}));
  }
  ...
  useEffect(()=>{
    if(user.uraniumMine) {
      ...
      let tm = URANIUM_MINE.TIMER - Math.floor((curSec - miningModule)/1000);
      if(tm<0) {
        setuTime(0);
      } else {
        setuTime(tm);
      ...
      >
        <Box sx={style}>
          ...
          <Box sx={{margin:"12px 0"}}>
            <p style={{display: "flex", justifyContent: "left", fontWeight: "700", width:"150px", margin:"auto"}}> <AllInclusiveIcon/> Earn: 400Gbaks</p>
          ...
          <Grid container sx={{justifyContent:"center"}}>
            <Grid sm={12} >
              ...
              >
                {showBuyButton && <Button variant="contained" color='primary' onClick={e => onBuyMining()} >Buy for {URANIUM_MINE.COST} SPX</Button>}
                {!showBuyButton && <>
                  ...
                  {requested ? (<>
                    ...
                    <>
                      <Button variant="contained" color='primary' onClick={e => onRequestMining()} >{URANIUM_MINE.REQUEST} Gbaks</Button>
                    </>
                  )}
